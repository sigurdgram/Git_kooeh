...
class_name Buildable_WishingTree
...
func can_interact(_interactor: Node2D):
    if ( not interactionSystem.is_limited_interaction() or interactionSystem.limited_interactables_contains(self)) and onStartWish.is_connected(start_wish):
        _canInteract = true
    else:
        _canInteract = false

...
class_name UI_Settings_Controls_KBM
...
func _ready():
    ...
    var bindings: Dictionary = settingsManager.get_pc_bindings()
    for binding in OrderedKeys:
        var spawnedEntry: UI_Settings_ControlRebindEntry_KBM = _keyBindEntryTemplate.instantiate()

extends UI_Widget
class_name UI_Settings_ControlRebindUI_KBM
...
var _ignoredControlTypes: Array = ["InputEventScreenDrag", "InputEventScreenTouch", "InputEventJoypadButton", "InputEventJoypadMotion", "InputEventMouseMotion"]
...
func _rebind():
    ...
    if _settingsManager.rebind_action_pc(_action, _currentEvent, _newEvent):
        UITree.PopWidgetFromLayer(self, _parentLayer.name)
    else:

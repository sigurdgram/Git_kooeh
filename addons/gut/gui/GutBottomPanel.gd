...
var _light_color = Color(0, 0, 0, 0.5)
...
func _ready():
    ...
    _ctrls.run_results_button.icon = get_theme_icon("AnimationTrackGroup", "EditorIcons")
...
func _on_results_bar_draw(bar):
    bar.draw_rect(Rect2(Vector2(0, 0), bar.size), Color(0, 0, 0, 0.2))
...
func load_result_output():
    ...
    if (summary_json.tests == 0):
        _light_color = Color(1, 0, 0, 0.75)
    elif (summary_json.failures != 0):
        _light_color = Color(1, 0, 0, 0.75)
    elif (summary_json.pending != 0):
        _light_color = Color(1, 1, 0, 0.75)
    else:
        _light_color = Color(0, 1, 0, 0.75)
